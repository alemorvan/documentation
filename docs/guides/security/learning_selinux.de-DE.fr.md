...
!!! Note As you can see, there is a `default` state (eg. at startup) and a running state.
...
!!! Warning Don't forget the `-P` option to keep the state after the next startup.
...
!!! Warning Disabling SELinux is done at your own risk! It is better to learn how SELinux works than to disable it systematically!
...
!!! Note `/etc/sysconfig/selinux` is a symlink to `/etc/selinux/config`
...
!!! Warning Beware of the SELinux mode change!
...
!!! Note If you are performing a context switch for a folder that is not standard for the system, creating the rule and then applying the context is a good practice as in the example below!
